...
//            if ($new_score<$score) {
            if ( ($new_score<$score) AND ($score-$new_score>$DELTA_M_MIN) ) {
                
                $delta_m=$score-$new_score;
                $min_delta_m=min($min_delta_m,$delta_m);
                
                $delta_r=($score>0.0)?number_format(($delta_m/$score)*100,2):'';
                my_log("\t\t OUT SCORE (".$new_score."<".$score." delta=".$delta_m." delta_r=".$delta_r.")\n", false);
                break;
            } elseif (($new_score>=$score) AND ($new_score-$score<$DELTA_P_MIN)) {
                
                $delta_p = $new_score - $score;
                $min_delta_p=min($min_delta_p,$delta_p);
             
                $delta_r=($score>0.0)?number_format(($delta_p/$score)*100,2):'';
                my_log("\t\t OUT SCORE (".$new_score.">".$score." delta=".$delta_p." delta_r=".$delta_r.")\n", false);
                break;
            }
...
*/
            ...
            my_log("\t\t IN new_score =".$score." delta=".$delta_p." delta_r=".$delta_r."\n", false);
            $greekSentIndex++;
        }
