...
for ($book=1; $book<=24; $book++) {
    $output = shell_exec('xsltproc --param book '.$book.' prova_with_milestones_2.xsl '.$iliadXML);
    //echo "OUTPUT(".$output.")\n"
    ...
    for ($i=0; $i<count($paras); $i++) {
        ...
        if ($i > 0 ) {
            ...
            $queryU="UPDATE Book_paras SET end=".$end.
                    " WHERE poem_id='".$iliadID."' AND book_id=".$book." AND start=".$para_starts[$i-1];
        ...
        $qI = $queryI." (".
                    "'". $iliadID ."', ". $book. ", ".$para_starts[$i].", '".$para."', ".$l_para
                   .")";
