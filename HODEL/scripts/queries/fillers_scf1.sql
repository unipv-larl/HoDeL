...
SELECT pl.root_id,
   ...
   AS CHAR(256) ) AS fillers_auxpc_coordapos_scf1,
     CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
   ...
   AS CHAR(256) ) AS fillers_verbo_scf1,
     CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
                            CONCAT( '(', pl.pathLabel1, ')', ft.lemma ),
   ...
   AS CHAR(256) ) AS afun_caso_modo_scf1,
  CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
   ...
   AS CHAR(256) ) AS caso_fillers_scf1,
   CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
   ...
   AS CHAR(256) ) AS caso_modo_fillers_scf1,
   CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
   ...
   AS CHAR(256) ) AS completo_nomodo_fillers_scf1,
   CAST(
              GROUP_CONCAT( IF( LENGTH(pl.pathLabel1) > 0 ,
   ...
   AS CHAR(256) ) AS completo_fillers_scf1
FROM
