...
(function($) {
	$.fn.dynamicForm = function(options) {
		var opts = $.extend({},
		$.fn.dynamicForm.defaults, options);
	...
	function addFieldset(options, addButtonObj) {
		...
		if (!options.maxFields || ((fieldsetElsLen + options.numTimes) <= options.maxFields)) {
			...
			db.click(function() { //add delete button event
				...
				parent.fadeOut(options.fadeDuration,
				function() {
					$(this).remove();
				});
				return false;
	...
	function getDeleteButtonObj(options) {
		var html = '<div class="' + options.deleteButtonDiv + '">' + '<input type="button"value="' + options.deleteButtonValue + '"' + ' class="' + options.deleteButtonClass + '" />' + '</div>';
		return $(html);
	}
